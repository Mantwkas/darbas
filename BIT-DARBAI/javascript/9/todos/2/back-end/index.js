...
//server.use - nustato pacius nustatymus kiekvinam requestu
...
//6. su "post" metodu: kai vartotojas uzsiregistruoja, jo duomenys saugomi masyve (kovertuojami i objekta)
const users = []; //masyvas (sudarytas is objektu) 
// gaudami duomeneys -> konvertuojami i objekta -> priskirti kiekvienam id (serveris priima duomenys ir pats priskiria Id(array.lenght+1) kai masyvas papushinamas i array)  
...
//Is kliento puses atliekams post request i register end point/server
//kai register forma uzpildoma- duomenys siunciami i serveri ir sukuriamas vartotojo objektas (masyve "users")
server.post("/register", (req, res )=> { // /register endopinte gausime vartotojo registracijos parametrus. gausime req.body - objektas grazinamas kai siunciami is kliento duomeny serveriui
...
server.post("/login", (req, res) => {
    ...
    //3.  toliau tikrinama ar slaptazodis atitinka. Jei taip: siu nciamas atsakymas is serverio "sekmingai prisijungiate"
        if (selectedUser.password === password) { 
            ...
            res.status(200).json({url: "http://127.0.0.1:5500/2024.01.10/front-end/todos.html" }); // po prisijungimo redirectina i kita page
        }
...
//10. CRUD operacijos  for todos
server.post("/todos", (req, res) => { //kuriamas naujas todo
    const {username, todo} = req.body; //todo susidaro is 2 laukeliu
    //validation
    ...
    const selectedUser = users.find((user) => user.username.toLowerCase() === username); //kiekvienojs iteracijoj turesim po vartotoja (user) ir ziuresim ar pateiktas (is imput) vartotojo vardas "username" egzistuoja bet viename vartotuje "user.username"
    ...
    rest.status(201).json({message: "naujas todo buvo sekmingai pridetas", newTodo}); //grazinti json objektas 
})

//perskaityti/gauti visus todo
server.get("/todos", (req, res) => { 
    res.status(200).json(todos);  //grazinamas todo listas is "const todos = []; ")
})
//perskaityti/gauti tam tikra todo
server.get("/todos", (req, res) => { 
    res.status(200).json(newTodo);  //anksciau sukurtas newTodo?
})

//PUT: atnaujinti egzistuojancio iraso duomenys (reikia gauti id ir laukelius kuriuos norima redaguoti)
server.put ("/todos/:id", (req, res) => { 
    const id = +req.params.id; //is parametru norima gauti id //id pareina string formatu tad prodedamas + kad paversti i skaiciu
...
const {username, todo} = req.body; //gaunami duomenys
...
//pagal id galima masyve pakiesti reiksme
todos[existingTodo] = {...todos[existingTodo],todo, username} //is todo saraso pagal existingTodo keiciamas pats todo
